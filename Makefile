...
bin/gondor-linux-amd64:
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 GOPATH=$(GOPATH) go build -ldflags "-X main.version=$(VERSION)" -o bin/gondor-linux-amd64 ./src

bin/gondor-darwin-amd64:
	GOOS=darwin GOARCH=amd64 CGO_ENABLED=0 GOPATH=$(GOPATH) go build -ldflags "-X main.version=$(VERSION)" -o bin/gondor-darwin-amd64 ./src

bin/gondor-windows-amd64.exe:
	GOOS=windows GOARCH=amd64 CGO_ENABLED=0 GOPATH=$(GOPATH) go build -ldflags "-X main.version=$(VERSION)" -o bin/gondor-windows-amd64.exe ./src
